...
// ========================================================
    ...
    public void ex01_accumulateFirstLetters() {
        ...
        input.stream().forEach(s -> sb.append(s.charAt(0)));
    ...
    public void ex04_stringifyMap() {
        ...
        input.forEach((s, i) -> sb.append(s).append(i));
    ...
    @Test
    public void ex04a_stringifyMap() {
        Map<String, Integer> input = new TreeMap<>();
        input.put("c", 3);
        input.put("b", 2);
        input.put("a", 1);
    ...
    public void ex05_mapOfStringLengths() {
        ...
        list.forEach(s -> result.merge(String.valueOf(s.charAt(0)), s.length(), (a,b) -> a+b));
...
// ========================================================
    ...
    public void ex06_upcaseOddLengthWords() {
        ...
        List<String> result = input.stream().filter(a -> a.length()%2 != 0)
    ...
    public void ex07_joinStreamRange() {
       ...
       String result = input.stream().skip(1).limit(4).map(a -> String.valueOf(a.charAt(1))).collect(Collectors.joining(","));
    ...
    public void ex10_findLongestLine() throws IOException {
    	String longest = reader.lines().max(Comparator.comparing(String::length)).get();
    ...
    public void ex12_listDifference() {
        ...
        List<Integer> result = IntStream.range(0, one.size())
        		.mapToObj(pos -> one.get(pos)-two.get(pos))
        		.collect(Collectors.toList());
...
// ========================================================
    ...
    public void ex13_stringsToCharacters() {
        ...
        List<Character> result = input.stream()
        		.flatMap(w -> w.chars().mapToObj(a -> (char)a))
        		.map(a -> (Character)a)
        		.collect(Collectors.toList());
    ...
    public void ex17_sortedLowerCaseDistinctByLengthThenAlphabetically() throws IOException {
        List<String> result = reader.lines().flatMap(line -> Stream.of(line.split(REGEXP)))
        		.map(String::toLowerCase)
        		.distinct()
    ...
    public void ex18_countTotalAndDistinctWords() {
    	Stream<String> words = reader.lines().flatMap(line -> Stream.of(line.split(REGEXP)));
    	LongAdder adder = new LongAdder();
    	long distinctCount = words
    			.map(String::toLowerCase)
    			.peek(s -> adder.increment())
    			.distinct().count(); // TODO
        long totalCount = adder.longValue(); // TODO
...
// ========================================================
    ...
    public void ex19_bigFactorial() {
        BigInteger result = LongStream.rangeClosed(1l, 21l)
        	.mapToObj(pos -> BigInteger.valueOf(pos))
